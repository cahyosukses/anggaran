...
<script type="text/javascript">
    ...
    $(function() {
        //show_ajax_indicator();
        get_list_prodi(1);
        $('#add_prodi').click(function() {
            reset_form();
            $('#datamodal').modal('show');
            $('#datamodal h4.modal-title').html('Tambah Prodi');
            tinyMCE.activeEditor.setContent('');
        });

        $('#reload_prodi').click(function() {
            reset_form();
            get_list_prodi(1);
        });
    });
    
    function get_list_prodi(p, id) {
        $('#form-pencarian').modal('hide');
        var id = '';
        $.ajax({
            type : 'GET',
            url: '<?= base_url("api/restrictarea/prodis") ?>/page/'+p+'/id/'+id,
            data: '',
            cache: false,
            dataType: 'json',
            beforeSend: function() {
                ...
                $.each(data.data,function(i, v){
                    ...
                    str = '<tr class="'+highlight+'">'+
                            ...
                            '<td align="center" class=aksi>'+
                                '<button type="button" class="btn btn-default btn-mini" onclick="edit_prodi(\''+v.id+'\')"><i class="fa fa-pencil"></i></button> '+
                                '<button type="button" class="btn btn-default btn-mini" onclick="delete_prodi(\''+v.id+'\','+data.page+');"><i class="fa fa-trash-o"></i></button>'+
                            '</td>'+
                        '</tr>';
    ...
    function edit_prodi(id) {
        $('#oldpict').html('');
        $('#datamodal').modal('show');
        $('#datamodal h4.modal-title').html('Edit Prodi');
        $.ajax({
            type: 'GET',
            url: '<?= base_url('api/restrictarea/prodis') ?>/page/1/id/'+id,
            dataType: 'json',
            success: function(data) {
    ...
    function konfirmasi_save() {
        $('#isi_prodi').val(tinyMCE.get('isi').getContent());
        bootbox.dialog({
    ...
    function save_prodi() {
        $.ajax({
            type: 'POST',
            url: '<?= base_url('api/restrictarea/prodi') ?>',
            dataType: 'json',
            ...
            buttons: {
              ...
              ya: {
                ...
                callback: function() {
                    $.ajax({
                        type: 'DELETE',
                        url: '<?= base_url('api/restrictarea/prodi') ?>/id/'+id,
                        dataType: 'json',
                        success: function(data) {
        ...
        <div class="col-md-12">
          <div class="grid simple ">
            <div class="grid-title">
              <h4>Daftar List Prodi</h4>
              <div class="tools"> 
                    <button id="add_prodi" class="btn btn-info btn-mini"><i class="fa fa-plus-circle"></i> Tambah</button>
                    <!--<button id="cari_button" class="btn btn-mini"><i class="fa fa-search"></i> Cari</button>-->
                    <button id="reload_prodi" class="btn btn-mini"><i class="fa fa-refresh"></i> Reload</button>
                </div>
            </div>
            <div class="grid-body">
                ...
                <div id="result">
                    <table class="table table-bordered table-stripped table-hover" id="load_data_table">
                        ...
                        <tr>
                          ...
                          <th width="55%" class="left">Nama Prodi</th>
    ...
    <div id="datamodal" class="modal fade">
      ...
      <div class="modal-content">
        ...
        <form id="formadd" method="post" role="form">
            ...
            <input type="hidden" name="isi_prodi" id="isi_prodi" />
            <div class="form-group">
                <label for="recipient-name" class="control-label">Nama Link:</label>
                <input type="text" name="link"  class="form-control" id="link">
            </div>
            <div class="form-group">
                <label for="recipient-name" class="control-label">Nama Prodi:</label>
